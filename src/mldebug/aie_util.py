...
class AIEUtil:
  ...
  def park_at_ofm_release(self, ofm_release_pc, count, sid, is_last_layer=False):
    """
    ...
    count is the expected number of releases. kernelWrapper's release is
    predicated, so a layer that releases less often than once per iteration
    will overshoot into the next layer; the final PC check reports that.
    """
